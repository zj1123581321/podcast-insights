export const meta = {
  name: 'extract-podcast-recommendations',
  description: '从播客校对转录稿提取主播的实地/好物/影视剧推荐为结构化JSON（多播客可复用）',
  phases: [{ title: 'Extract', detail: '每集一个Sonnet agent：自检跳过→读稿→提取→写JSON' }],
}

// 本步骤由 Claude Code 的 Workflow 工具运行（不是独立 python 脚本）。
// 通过 args 传参，便于跨播客复用：
//   args = { baseDir, hosts, total, vols? }
//     baseDir : 该播客数据目录绝对路径（含 transcripts/ 与 extracted/）
//     hosts   : 主播名数组，如 ["肥杰","惠子"]
//     total   : 总集数（默认 234）
//     vols    : 仅处理的集号子集（repair 时用；不传=全量 1..total）
//
// 默认值指向 feihua，方便直接试跑。
let a = args
...
const RULES = (vol) => {
  ...
  return `你在处理一档中文播客第 ${vol} 集的"校对转录稿"，提取两位主播（${hostList}）的【推荐内容】为结构化 JSON。
...
- 正文是带说话人标签的口语对话「${HOSTS[0]}：…」「${HOSTS[1] || '嘉宾'}：…」，推荐内容散落在闲聊中、无结构。
...
公共规则：
- recommender ∈ {${HOSTS.map((h) => '"' + h + '"').join('、')},"共同"}：按说话人标签判断；两人共同讨论且都认可填"共同"。
